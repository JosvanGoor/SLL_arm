#include <human_recognition/segment_2_cameras.h>
...
void SegmentHumans::getFeatures(PCLPointCloudPtr cloud, Human* human)
{
    ...
    float legs_total_r = 0;
    float legs_total_g = 0;
    float legs_total_b = 0;
    float legs_total_y = 0;
    float legs_total_wh = 0; 
    float legs_total_gr = 0;
    float legs_total_bl = 0;
    float upper_total_r = 0;
    float upper_total_g = 0;
    float upper_total_b = 0;
    float upper_total_y = 0;
    float upper_total_wh = 0; 
    float upper_total_gr = 0;
    float upper_total_bl = 0;
    float legs_n_points = 0;
    float upper_n_points = 0;
    ...
    for(int i=0; i < cloud->points.size(); i++)
    {
        ...
        float h, s, v;
        h = 0; // In case a precision error occurs and H gets evaluated when the color should be dependent only on S or V.
        ...
        if (cloud->points[i].z < leg_height)
        {
            ...
            {
                /*legs_total_r += 0.5 * computeColourPortion(h, H_RED_LOW_CENTER, H_RED_LOW_CENTER, H_YELLOW_CENTER);
                legs_total_r += 0.5 * computeColourPortion(h, H_RED_HIGH_CENTER, H_BLUE_CENTER, H_RED_HIGH_CENTER);
                legs_total_y += computeColourPortion(h, H_YELLOW_CENTER, H_RED_LOW_CENTER, H_GREEN_CENTER);
                legs_total_g += computeColourPortion(h, H_GREEN_CENTER, H_YELLOW_CENTER, H_BLUE_CENTER);
                legs_total_b += computeColourPortion(h, H_BLUE_CENTER, H_GREEN_CENTER, H_RED_HIGH_CENTER);*/
                legs_total_r += 0.5 * computeColourPortion(h, H_RED_LOW_CENTER, H_RED_LOW_CENTER, H_RED_YELLOW);
                legs_total_r += 0.5 * computeColourPortion(h, H_RED_HIGH_CENTER, H_BLUE_RED, H_RED_HIGH_CENTER);
                legs_total_y += computeColourPortion(h, H_YELLOW_CENTER, H_RED_YELLOW, H_YELLOW_GREEN);
                legs_total_g += computeColourPortion(h, H_GREEN_CENTER, H_YELLOW_GREEN, H_GREEN_BLUE);
                legs_total_b += computeColourPortion(h, H_BLUE_CENTER, H_GREEN_BLUE, H_BLUE_RED);
            }
		...
		else
		{
            ...
            {
                /*upper_total_r += 0.5 * computeColourPortion(h, H_RED_LOW_CENTER, H_RED_LOW_CENTER, H_YELLOW_CENTER);
                upper_total_r += 0.5 * computeColourPortion(h, H_RED_HIGH_CENTER, H_BLUE_CENTER, H_RED_HIGH_CENTER);
                upper_total_y += computeColourPortion(h, H_YELLOW_CENTER, H_RED_LOW_CENTER, H_GREEN_CENTER);
                upper_total_g += computeColourPortion(h, H_GREEN_CENTER, H_YELLOW_CENTER, H_BLUE_CENTER);
                upper_total_b += computeColourPortion(h, H_BLUE_CENTER, H_GREEN_CENTER, H_RED_HIGH_CENTER);*/
                upper_total_r += 0.5 * computeColourPortion(h, H_RED_LOW_CENTER, H_RED_LOW_CENTER, H_RED_YELLOW);
                upper_total_r += 0.5 * computeColourPortion(h, H_RED_HIGH_CENTER, H_BLUE_RED, H_RED_HIGH_CENTER);
                upper_total_y += computeColourPortion(h, H_YELLOW_CENTER, H_RED_YELLOW, H_YELLOW_GREEN);
                upper_total_g += computeColourPortion(h, H_GREEN_CENTER, H_YELLOW_GREEN, H_GREEN_BLUE);
                upper_total_b += computeColourPortion(h, H_BLUE_CENTER, H_GREEN_BLUE, H_BLUE_RED);
            }
    ...
    human->legs_red 	= legs_total_r / legs_n_points;
    human->legs_green 	= legs_total_g / legs_n_points;
    human->legs_blue 	= legs_total_b / legs_n_points;
    human->legs_yellow 	= legs_total_y / legs_n_points;
    human->legs_black 	= legs_total_bl / legs_n_points;
    human->legs_white 	= legs_total_wh / legs_n_points;
    human->legs_grey 	= legs_total_gr / legs_n_points;

    human->upper_red 	= upper_total_r / upper_n_points;
    human->upper_green 	= upper_total_g / upper_n_points;
    human->upper_blue 	= upper_total_b / upper_n_points;
    human->upper_yellow = upper_total_y / upper_n_points;
    human->upper_black 	= upper_total_bl / upper_n_points;
    human->upper_white 	= upper_total_wh / upper_n_points;
    human->upper_grey 	= upper_total_gr / upper_n_points;
}
