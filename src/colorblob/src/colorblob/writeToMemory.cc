...
void Colorblob::writeToMemory(vector<cv::Rect>& obs, string name, colorblob::BboxArray& message)
{
  
  message.size = obs.size();
...
// 		pt_observations.put(index_str, pt_obs_buffer.str());
	...
	//stri*///ng observations_dict = pt_observations_buffer.str();
